<template>
    <div class="register-container">
        <el-form size="large" :model="form" ref="ruleFormRef" :rules="rules" class="register-form">
            <h3 class="register-title  e-primary-text">{{ $t('common.appName') }} - {{ $t('register.register') }}</h3>
            <!-- <el-form-item class="register-input" prop="email">
                <el-input prefix-icon="Message" placeholder="请输入邮箱" v-model="form.email" type="text"></el-input>
            </el-form-item> -->
            ...
            <el-form-item>
                <el-button type="primary" @click="summit(ruleFormRef)" class="register-button">{{$t('register.commitRegister')}}</el-button>
...
function summit(ruleFormRef) {
    ...
    ruleFormRef.validate((valid) => {
        if (valid) {
            apiRegister.registerSubmit(form.username, form.passwd, form.email, form.mailcode, () => {
                ElMessage({
                    // duration: 5000,
                    showClose: true,
                    dangerouslyUseHTMLString:true,
                    type: "success",
                    message: $18n.t("register.registerSuccess")
                })
                setTimeout(() => {
                    goToLogin()
                }, 2000);
...
function sendmail(ruleFormRef) {
    ...
    ruleFormRef.validate((valid) => {
        if (valid) {
            ...
            apiRegister.vcodeRegister(form.username, form.passwd, form.email,()=>{
